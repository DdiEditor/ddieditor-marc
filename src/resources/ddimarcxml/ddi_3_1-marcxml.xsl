    ...
    <xsl:template match="s:StudyUnit">
        <record xmlns="http://www.loc.gov/MARC21/slim">
            ...
            <controlfield tag="008">
                ...
                <xsl:variable name="current" select="current-date()"/>
                <xsl:value-of select="year-from-date($current)"/>
                <xsl:value-of select="month-from-date($current)"/>
                <xsl:value-of select="day-from-date($current)"/>
